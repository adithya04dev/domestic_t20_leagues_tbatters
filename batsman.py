...
class Batsmant():
    ...
    def create_df(self,leagues,team,overs,BowlingType,Season):
            ...
            balls=len(self.df.loc[(self.df['is_wide']==0)  & (self.df['is_noball']==0) & (self.df["LeagueName"].isin(leagues))  & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season)) & (self.df['BattingTeam'] == team) ] )
            out = len(   self.df.loc[(self.df["BattingTeam"] == team) & (self.df["LeagueName"].isin(leagues)) & (self.df["player_out"]==1)  & (self.df["BowlingType"].isin(BowlingType))   & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
...
# result=bat.calculate(['TNPL'],'Chepauk Super Gillies',[1,2,3,4],["Pace","Spin"],[2023])
# # #print(result)
# result1=bat.overall()
# # print(result1['strike_rate'])
# print(result1['balls_played'])
#
#
# with open('C:/Users/adith/Documents/ds/t20_leagues/set_3_leagues/app/teams/batting/batting1.pkl', 'wb') as f:
#     pickle.dump(bat, f)